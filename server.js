...
const {retrieveArray, User, Address_book, Manufacturer, Item, Cart, Favourite_item, Store, Favourite_store, Order, Items_within_store} = require('./utils')
...
let address_books = createRandomAddressBookService(1); // FK: user_id -> id (user)
let manufacturers = createRandomManufacturerService(1); // PRIMARY
let items = createRandomItemService(15); // FK: manufacturer_id -> id (manufacturer)
let carts = createRandomCartService(1); // FK: user_id -> id (user), FK: item_id -> id (item)
let favourite_items = new Array(1); // doesn't have its own generator because this table is comprised of FK's only
let stores = createRandomStoreService(1); // PRIMARY
let favourite_stores = new Array(1); // doesn't have its own generator because this table is comprised of FK's only
let orders = createRandomOrderService(3); // FK: user_id -> id (user)
